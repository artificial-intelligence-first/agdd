...
def get_worktree_settings() -> WorktreeSettings:
    ...
    root = _resolve_root(_get_env("MAGSAG_WORKTREES_ROOT"))
    # Create the directory eagerly so downstream code can rely on existence.
    root.mkdir(parents=True, exist_ok=True)

    max_concurrency_raw = _get_env("MAGSAG_WT_MAX_CONCURRENCY", "8") or "8"
    try:
        max_concurrency = max(1, int(max_concurrency_raw))
    except ValueError as exc:
        raise ValueError(
            f"Invalid MAGSAG_WT_MAX_CONCURRENCY value: {max_concurrency_raw!r}"
        ) from exc

    ttl_spec = _get_env("MAGSAG_WT_TTL", "14d") or "14d"
