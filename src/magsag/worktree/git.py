...
def _to_info(data: Mapping[str, object], extras: Mapping[str, str]) -> WorktreeInfo:
    ...
    lock_reason_value = data.get("lock_reason")
    prunable_reason_value = data.get("prunable_reason")

    return WorktreeInfo(
        path=Path(path_value),
        head=head_value if isinstance(head_value, str) else None,
        branch=branch_value if isinstance(branch_value, str) else None,
        git_dir=Path(gitdir) if isinstance(gitdir, str) else None,
        is_bare=bool(data.get("is_bare", False)),
        is_detached=bool(data.get("is_detached", False)),
        locked=bool(data.get("locked", False)),
        lock_reason=lock_reason_value if isinstance(lock_reason_value, str) else None,
        prunable=bool(data.get("prunable", False)),
        prunable_reason=prunable_reason_value if isinstance(prunable_reason_value, str) else None,
