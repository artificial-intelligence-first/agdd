...
class TestAsyncRequirement:
    ...
    @pytest.mark.asyncio
    async def test_sync_skill_raises_error(self, skill_runtime, mock_registry):
    ...
    @pytest.mark.asyncio
    async def test_async_skill_works(self, skill_runtime, mock_registry):
    ...
    def test_sync_agent_raises_error(self, agent_runner, mock_registry, mock_observer):
        """Test that synchronous agents raise ValueError."""
        # Create a synchronous agent
        def sync_agent(payload, **kwargs):
            return {"result": "sync"}
    ...
    def test_async_agent_works(self, agent_runner, mock_registry, mock_observer):
        """Test that async agents work correctly."""
        # Create an async agent
        async def async_agent(payload, **kwargs):
            return {"result": "async"}
        ...
        result = agent_runner.run_agent("test-async-agent", {"test": "data"}, observer=mock_observer)
        assert result["result"] == "async"

    @pytest.mark.asyncio
    async def test_sync_skill_with_mcp_param_raises_error(self, skill_runtime, mock_registry):
        """Test that sync skills with MCP parameter raise appropriate error."""
        # Create a synchronous skill that expects MCP
        def sync_skill_with_mcp(payload, mcp=None):
    ...
    def test_error_message_is_helpful(self, skill_runtime, mock_registry):
        """Test that error messages for sync skills are helpful."""
        # Create a synchronous skill
        def problematic_sync_skill(payload):
            return {"result": "sync"}
