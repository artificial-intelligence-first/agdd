...
- **Pluggable Storage Layer (Phase 1)**
  ...
  - Storage observability logger (`StorageObservabilityLogger`) as modern replacement for file-based logging
  - CLI commands for data management:
    - `agdd data init` - Initialize storage backend
    - `agdd data query` - Query runs and events
    - `agdd data search` - Full-text search across events
    - `agdd data vacuum` - Clean up old data with retention policies
    - `agdd data archive` - Archive to external storage (S3/MinIO)
